...
/*添加事件*/
void event_add(int epollfd,int events,struct myevents *ev);
void recvdata(int fd,int events,void *arg);
void senddata(int fd,int events,void *arg);
void event_del(int epollfd,struct myevents *ev);

int main(int argc, char  *argv[])
{
    unsigned short port = SERVER_PORT;   //默认端口号
    if(argc==2){
        port = atoi(argv[1]);          //使用用户指定的端口号
    }
